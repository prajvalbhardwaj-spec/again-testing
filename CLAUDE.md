# CLAUDE.md — Project Rules for FastAPI Backend

These rules must be followed in every session, every feature, and every file generated.
...
- `render.yaml` build/start commands must run `seed.py` FIRST:
  ```yaml
...
- At the VERY START, before generating any code, ask ONLY for the GitHub repository URL.
